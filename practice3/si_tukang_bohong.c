...
void check(AVLNode* node, AVLNode* prev, int sum, int value){
    if(node == NULL) return;
    if(found) return;
    sum = sum + node->data;
    if(sum == value) found = true;
    else{
        if(node->left != prev) check(node->left, node, sum, value);
        if(node->right != prev) check(node->right, node, sum, value);
        if(node->parent != prev) check(node->parent, node, sum, value);
    }
}

void traverse(AVLNode* node, int value){
    if(node == NULL) return;
    if(found) return;
    if(node->left) check(node->left, node, 0, value);
    if(node->right) check(node->right, node, 0, value);
    if(node->parent) check(node->parent, node, 0, value);
...
int main(){
    ...
    while(n--){
        scanf("%d %d", &c, &x);
        if(c == 1) avl_insert(&avl, x);
        else if(c == 2){
            found = false;
            traverse(avl._root, x);
            if(found) printf("Aaa udh lama bgt ga diboongin:)\n");
